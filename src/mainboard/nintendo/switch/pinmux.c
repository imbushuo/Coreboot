...
void pinmux_set_config_r(int pin_index, uint32_t config)
{
	pinmux_regs[pin_index] = config;
}

uint32_t pinmux_get_config_r(int pin_index)
{
...
{
	uint32_t reg = pinmux_get_config_r(pin_index);
	...
	pinmux_set_config_r(pin_index, reg);
...
void pinmux_config_i2c(int i2c_index)
{
	...
	pinmux_set_config_r(pinmux_i2c[i2c_index*2+0], PINMUX_INPUT_ENABLE);
	pinmux_set_config_r(pinmux_i2c[i2c_index*2+1], PINMUX_INPUT_ENABLE);
...
void pinmux_config_uart(int uart_index)
{
	...
	pinmux_set_config_r(pinmux_uart[uart_index*4+0], PINMUX_PULL_UP);
	pinmux_set_config_r(pinmux_uart[uart_index*4+1], PINMUX_INPUT_ENABLE | PINMUX_PULL_UP);
	pinmux_set_config_r(pinmux_uart[uart_index*4+2], PINMUX_PULL_UP);
	pinmux_set_config_r(pinmux_uart[uart_index*4+3], PINMUX_INPUT_ENABLE | PINMUX_PULL_UP);
